...
- Credentials are resolved in a predictable order: explicit config → environment →
  ...
  IAM-style role providers where available. The IAM metadata probe is
  timeout-bounded, so off-cloud it fails fast rather than hanging. This supports
  short-lived cloud credentials instead of static keys. lode does not invent its
